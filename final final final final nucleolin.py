...
gray_palette = ['lightgray', 'darkgray']
...
axes.flat[3].bar(['Isogenic', 'C9'],
                 ...
                 color=['lightgray', 'darkgray'], width=0.25)
...
if 'aggregates_df' in locals() and not aggregates_df.empty and 'mean_intensity' in aggregates_df.columns:
    ...
    if len(c9_agg_int) >= 3 and len(iso_agg_int) >= 3:
        ...
        plt.figure(figsize=(4, 5))  # ← Narrower
        ax = sns.boxplot(data=agg_image_summary, x='condition', y='mean_intensity',
                         palette=gray_palette, order=['Isogenic', 'C9'], width=0.25)
        sns.stripplot(data=agg_image_summary, x='condition', y='mean_intensity',
                      color='black', size=6, alpha=0.8, order=['Isogenic', 'C9'])  # ← Larger, per-image dots
...
if not ratio_per_image.empty:
    ...
    if 'C9' in available_conditions and 'Isogenic' in available_conditions:
        ...
        if len(c9_vals) >= 3 and len(iso_vals) >= 3:
            u, p = mannwhitneyu(c9_vals, iso_vals, alternative='two-sided')
    ...
    if plot_order and color_palette:
        if len(plot_order) > 1:
            ...
            sns.stripplot(data=ratio_per_image, x='condition', y='agg_nuc_area_ratio',
                          color='black', size=6, order=plot_order, ax=ax)  # ← per-image
...
if not aggregates_df.empty and 'aggregate_area_um2' in aggregates_df.columns:
    ...
    if len(c9_aggregates) >= 3 and len(iso_aggregates) >= 3:
        ...
        # Statistical test
        u_stat, p_val = mannwhitneyu(c9_sizes, iso_sizes, alternative='two-sided')
        ...
        ax = sns.boxplot(data=aggregates_df, x='condition', y='aggregate_area_um2',
                         palette=pastel_palette, order=condition_order, width=0.6)
        sns.stripplot(data=aggregates_df, x='condition', y='aggregate_area_um2',
                      color='black', size=3, alpha=0.5, order=condition_order)
        ...
        # Also create a bar plot showing mean ± SEM
        ...
        bars = plt.bar(x_pos, means, yerr=sems, capsize=10,
                       color=[pastel_palette['Isogenic'], pastel_palette['C9']],
                       width=0.6)

        # Add individual data points
        plt.scatter(np.zeros(len(iso_sizes)) + np.random.normal(0, 0.05, len(iso_sizes)),
                    iso_sizes, color='black', alpha=0.4, s=20)
        plt.scatter(np.ones(len(c9_sizes)) + np.random.normal(0, 0.05, len(c9_sizes)),
                    c9_sizes, color='black', alpha=0.4, s=20)
        ...
        if p_val < 0.05:
            ...
            plt.ylim(0, y_max)
            plt.text(0.5, max(means) * 1.08, p_stars,
                     ha='center', va='bottom', fontsize=12, fontweight='bold')
        ...
        print(f"\n⚠ Insufficient data for aggregate size comparison:")
        print(f"   C9: {len(c9_aggregates)} aggregates (need ≥3)")
        print(f"   Isogenic: {len(iso_aggregates)} aggregates (need ≥3)")
else:
    print("\n⚠ No aggregate size data available for comparison")
# ============================================================================
    ...
    if len(available_conditions) > 0:
        ...
        if len(plot_order) > 1:
            sns.boxplot(data=image_summary, x='condition', y='mean_intensity',
                        palette=color_palette, width=0.25, order=plot_order)
            sns.stripplot(data=image_summary, x='condition', y='mean_intensity',
                          color='black', size=6, alpha=0.8, order=plot_order)
        else:
            sns.boxplot(data=image_summary, x='condition', y='mean_intensity',
...
# =============================================================================
# STATISTICAL COMPARISON PLOTS (Mann-Whitney) — now also use per-image data
# =============================================================================
...
if not image_summary.empty and 'nuclear_area_um2' in image_summary.columns:
    ...
    if 'C9' in available_conditions and 'Isogenic' in available_conditions:
        ...
        if len(c9_nuc_size) >= 3 and len(iso_nuc_size) >= 3:
            ...
            plt.figure(figsize=(4, 5))  # ← Narrower
            ax = sns.boxplot(data=image_summary, x='condition', y='nuclear_area_um2',
                             palette=gray_palette, order=['Isogenic', 'C9'], width=0.25)
            sns.stripplot(data=image_summary, x='condition', y='nuclear_area_um2',
                          color='black', size=6, alpha=0.8, order=['Isogenic', 'C9'])
...
if not image_summary.empty and 'mean_intensity' in image_summary.columns:
    c9_nuc_int = pd.to_numeric(image_summary[image_summary['condition'] == 'C9']['mean_intensity'], errors='coerce').dropna()
    iso_nuc_int = pd.to_numeric(image_summary[image_summary['condition'] == 'Isogenic']['mean_intensity'], errors='coerce').dropna()
    if len(c9_nuc_int) >= 3 and len(iso_nuc_int) >= 3:
        ...
        plt.figure(figsize=(4, 5))  # ← Narrower
        ax = sns.boxplot(data=image_summary, x='condition', y='mean_intensity',
                         palette=gray_palette, order=['Isogenic', 'C9'], width=0.25)
        sns.stripplot(data=image_summary, x='condition', y='mean_intensity',
                      color='black', size=6, alpha=0.8, order=['Isogenic', 'C9'])
