...
conditions = []
for f in nd2_files:
    if "BC9" in f or "Bc9" in f:
        conditions.append("C9")
    elif "BISO" in f or "BIso" in f or "Isogenic" in f:
...
for ax, metric, title in zip(axes.flat[:3], plot_metrics, plot_titles):
    sns.boxplot(data=image_metrics, x='condition', y=metric, ax=ax, palette=['salmon', 'lightgreen'])
    sns.stripplot(data=image_metrics, x='condition', y=metric, ax=ax, color='black', size=4)
    ax.set_title(title)
    ax.set_xlabel("")

# Aggregates per nucleus (population-level)
axes.flat[3].bar(['C9', 'Isogenic'],
...
sns.boxplot(data=image_metrics, x='condition', y='frac_nucleolin_positive', palette=['salmon', 'lightgreen'])
sns.stripplot(data=image_metrics, x='condition', y='frac_nucleolin_positive', color='black', size=4)
...
sns.boxplot(data=image_metrics, x='condition', y='frac_cells_with_aggregates', palette=['salmon', 'lightgreen'])
sns.stripplot(data=image_metrics, x='condition', y='frac_cells_with_aggregates', color='black', size=4)
...
if not aggregates_df.empty and 'mean_intensity' in aggregates_df.columns:
    ...
    if 'aggregate_area_um2' in aggregates_df.columns:
        ...
        sns.boxplot(data=agg_size_per_image, x='condition', y='mean_aggregate_size', palette=['salmon', 'lightgreen'])
        sns.stripplot(data=agg_size_per_image, x='condition', y='mean_aggregate_size', color='black', size=4)
        ...
        c9_vals = agg_size_per_image[agg_size_per_image['condition'] == 'C9']['mean_aggregate_size']
        iso_vals = agg_size_per_image[agg_size_per_image['condition'] == 'Isogenic']['mean_aggregate_size']
        if len(c9_vals) >= 3 and len(iso_vals) >= 3:
            u_stat, p_val = mannwhitneyu(c9_vals, iso_vals, alternative='two-sided')
            d_agg_size = cliffs_delta(c9_vals, iso_vals)
            print(f" mean aggreagate size Cliff's d = {d_agg_size:.3f}")
            print(f"\nMean aggregate size: U={u_stat:.1f}, p={p_val:.4f}")
        else:
            print("\nMean aggregate size: insufficient data for test")
else:
    print("\nSkipping aggregate analysis: no aggregates detected.")
...
if not agg_positive_area.empty:
    plt.figure(figsize=(6, 5))
    sns.boxplot(data=agg_positive_area, x='condition', y='agg_nuc_area_ratio', palette=['salmon', 'lightgreen'])
    sns.stripplot(data=agg_positive_area, x='condition', y='agg_nuc_area_ratio', color='black', size=4)
...
# 1. AGGREGATE INTENSITY (per aggregate)
if 'aggregates_df' in locals() and not aggregates_df.empty and 'mean_intensity' in aggregates_df.columns:
    plt.figure(figsize=(6, 5))
    sns.boxplot(data=aggregates_df, x='condition', y='mean_intensity',
                palette=['salmon', 'lightgreen'], width=0.6)
    sns.stripplot(data=aggregates_df, x='condition', y='mean_intensity',
                  color='black', size=3, alpha=0.5)
    plt.ylabel("Aggregate Intensity (A.U.)", fontsize=12)
    plt.xlabel("Condition", fontsize=12)
    plt.title("Aggregate Intensity: C9 vs Isogenic", fontsize=14, pad=15)
    plt.tight_layout()
    plt.savefig('main_plot_aggregate_intensity.png', dpi=300, bbox_inches='tight')
    plt.show()
else:
    print("⚠ Skipping Aggregate Intensity plot: aggregates_df missing or empty")
...
if 'nuclear_area_um2' in df_all.columns and 'condition' in df_all.columns:
    ...
    if len(c9_nuc_size) >= 3 and len(iso_nuc_size) >= 3:
        ...
        # Plot
        plt.figure(figsize=(6, 5))
        sns.boxplot(data=df_all, x='condition', y='nuclear_area_um2', palette=['salmon', 'lightgreen'])
        sns.stripplot(data=df_all, x='condition', y='nuclear_area_um2', color='black', size=4, alpha=0.6)
    ...
    print("Nuclear size: column 'nuclear_area_um2' not found")
...
if 'mean_intensity' in df_all.columns and 'condition' in df_all.columns:
    ...
    if len(c9_nuc_int) >= 3 and len(iso_nuc_int) >= 3:
        ...
        # Plot
        plt.figure(figsize=(6, 5))
        sns.boxplot(data=df_all, x='condition', y='mean_intensity', palette=['salmon', 'lightgreen'])
        sns.stripplot(data=df_all, x='condition', y='mean_intensity', color='black', size=4, alpha=0.6)
